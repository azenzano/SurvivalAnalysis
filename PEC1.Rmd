...
Si verificamos la Ecuación $\eqref{eq:Fcond21}$, quedaría tal que
...
En la Figura \@ref(fig:fdensi) representamos la función de distribución $f(t)$ (ver Ecuación $\eqref{eq:ftEj1}$), para valores de $t$ de 0 a 100. En la Tabla \@ref(tab:tej1) vemos un ejemplo de los primero 10 valores del conjunto de datos calculados:
...
```{r fdensi, fig.cap = "Función de densidad f(t) para valores de t de 0 a 100.", fig.width=5, fig.height=4}
...
Vemos que a partir de la Ecuación $\eqref{eq:ftEj1}$, para $t < 0$, la función de distribucion es $F(t)=0$, y para $t \geq 0$ la función de distribución, $F(t)$ se puede expresar tal que
...
Si representamos la función de distribución por partes, $F(t)$ de manera completa tiene el siguientes aspecto:
...
```{r fdistri, fig.cap = "Función de distribución F(t) y función de supervivencia S(t) para valores de t de 0 a 100.", fig.width=5, fig.height=4}
...
```{r friesgo, fig.cap = "Función de riesgo instantáneo h(t) para valores de t de 0 a 100.", fig.width=5, fig.height=4}
...
```{r friesgoacum, fig.cap = "Función de riesgo acmulado H(t) para valores de t de 0 a 100.", fig.width=5, fig.height=4}
...
Haciendo uso de la ecuación de supervivencia $\eqref{eq:StEj1}$, la ecuación de riesgo instantaneo $\eqref{eq:htEj11}$, de riesgo acumulado $\eqref{eq:HtEj1}$ y siendo $f(t)=\frac{d}{dt}F(t)=-\frac{d}{dt}S(t)$ y $S(t)=1-F(t)$ [@UPC], la función de riesgo instantaneo, $h(t)$ se puede expresar tal que:
...
y volviendo sobre la Ecuación $\eqref{eq:StCompEj1}$, vemos que la igualdad se cumple como se puede ver a continuación.
...
```{r fsuper, fig.cap = "Función de supervivencia S(t) y función de comprobación de supervivencia S(t)=-log(H(t))  para valores de t de 0 a 100.", fig.width=5, fig.height=4}
...
$h(t)$ representa la función de riesgo o también la tasa de fallo en el instante $t$, y puede interpretarse como la "velocidad" a la cual se producen los "eventos" o "fallos" en el instante $t$ [@UPC]. En en instante, $t=2$ que estamos analizando la tasa de fallo instantanea es relativamente alta, por lo que podríamos deducir que estamos ante una etapa de "mortalidad infantil".
...
\begin{equation}
  =\int{\frac{1}{1+\frac{sin^2(u)}{cos^2(u)}}}\frac{1}{cos^2(u)}du=\int{\frac{1}{\frac{cos^2(u)+sen^2(u)}{cos^2(u)}}}\frac{1}{cos^2(u)}du
\end{equation}
...
Podemos responder a esta pregunta hallando el número de individuos que ha sobrevido mediante la función de supervivencia, $S(t)$ expresanda mediante la Ecuación $\eqref{eq:StEj1}$, y que es complementaria de $F(t) = P(T \leq t)$, para el instante, $t = 1$.
...
f_km_estimator <- function(df,p_g){
    ...
    #df_coinci = df_merge %>% filter(!is.na(failure))
    #df_no_coinci = df_merge %>% filter(is.na(failure)) %>% mutate(link=1)
...
El tiempo medio hasta el fallo que coincide con área encerrada entre la función $S(t)$ y el semieje horizontal $t$ en el intervalo $\left[0,\infty\right)$ [@UPC]. Lo podemos calcular mediante la fórmula del MTTF [@UPC].



```{r echo=TRUE}
vida_mediaA = sum(df_A$S_1t)
```
...
```{r echo=TRUE}
vida_mediaB = sum(df_B$S_1t)
```

La vida media del grupo B es `r vida_mediaB`.
...
En este apartado describiremos y evaluaremos las equaciones para analizar si las curvas de Kaplan-Meier de los 2 grupos son estadísticamente equivalentes o no. Para ello, primero utilizaremos el metódo log-rank y después el resto de estadísticos alternativos. Útilizaremos el test de hipótesis para verificar si son significativamente similares o no. 

La hipotésis nula $H_{0}$ que es testada, es que no hay diferencias significativas entre las dos curvas de supervivencia.

Bajo esta hipótesis nula, el estadístico es aproximadamente igual a $\chi^2$ con un grado de libertad. El p-valor se puede determinar a través de las tablas de la distribución de $\chi^2$ o traves de la función de `R:pchisq`. Usaremos el valor $5\%$ (p-valor<0.05) para determinar si observamos una desviación significativa.  Cualquier desviación mayor que este nivel nos haría rechazar la hipótesis nula. Es decir, si el valor calculado para $\chi^2$ es mayor que el valor crítico convenido, entonces rechazaremos la hipótesis nula, y si no, la aceptaremos.


A continuación se muestran las equaciones de manera general [@KleinbaumDavidG2005Sa],[@Kosinski],[@Dardis2018], incluyendo el peso $w_{f}$. 
...
Por lo tanto, según este test, aceptamos la hipótesis nula $H_O$, y podemos afirmar de manerar significativa que las curvas de los dos grupos A y B son similares.
...
Como está expuesto en la literatura [@Kosinski],[@Dardis2018], además del Long-rank hay un familia de test basada en las Equaciones $\eqref{eq:LRtest}$, $\eqref{eq:LRtestU}$ y $\eqref{eq:LRtestV}$, con los pesos $w_{f_{t}}$ descritos a continuación:
...
Por lo tanto, según este test, aceptamos la hipótesis nula $HO$, y podemos afirmar de manerar significativa que las curvas de los dos grupos A y B son similares.
...
Por lo tanto, según este test, aceptamos la hipótesis nula $H_O$, y podemos afirmar de manerar significativa que las curvas de los dos grupos A y B son similares. Cabe destacar, que con estos parámetros selecciónados ($p=9$, $p=3$) obtenemos un valor para el estadístico menor, que el calculado par el mismo estadístico con los parametros ($p=9$, $p=3$)
...
Utilizando las funciones `Surv`, `survfit` y  `comp` de `R` vemos que son los mismos a los obtenidos manualmente (ver Tabla \@ref(tab:talltests)).
...
Viendo la Tabla \@ref(tab:talltests) de resultados final, podemos decir que todos los tests dan un p-valor que indica que la hipóstesis nula $H_0$ debería de ser aceptada, por lo que podemos afirmar de manerar significativa que las curvas de los dos grupos A y B son similares. No obstante, parece claro que el conjunto de datos estudio no es muy elevado.



# Anexos

https://github.com/azenzano/SurvivalAnalysis.git (last version: 2022/04/10)
